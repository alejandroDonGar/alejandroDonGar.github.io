        ...
        try {
            ...
            container.innerHTML = `
                <div style="text-align:center; padding:3rem; background: rgba(239, 68, 68, 0.1); border-radius: 20px; border: 1px solid rgba(239, 68, 68, 0.2);">
                    ...
                    <button onclick="location.reload()" class="btn-primary" style="background:#ef4444; border:none;">
    ...
    function createGitHubProfilePreview() {
        return `
            <div class="repo-profile-readme-preview repo-profile-overview-preview repo-profile-compact-preview" aria-label="Preview visual del README principal de GitHub">
                <div class="repo-profile-topbar">
                    <span>alejandroDonGar / README.md</span>
                    <i class="fa-solid fa-pen"></i>
                </div>

                <div class="repo-profile-readme-body repo-profile-overview-body repo-profile-compact-body">
                    ...
                    <div class="repo-profile-content repo-profile-overview-content">
    ...
    function createCentroPlusApiPreview() {
        return `
            <div class="repo-centroplus-connect-preview" aria-label="Preview visual de CentroPlus Connect: dashboard, API REST y Swagger">
                <div class="centroplus-preview-bg" aria-hidden="true"></div>
    ...
    function renderRepos(repos) {
        ...
        repos.forEach(repo => {
            ...
            card.className = (hasCustomPreview ? `repo-card repo-card-featured-net${zeeBoard ? ' repo-card-featured-zee' : ''}` : 'repo-card') + ' repo-card-reveal';
            ...
            card.innerHTML = `
                <div class="repo-info">
                    <h3 style="font-size: 1.75rem; margin-bottom: 1rem;">${repo.name}</h3>
                    <p style="color: var(--text-muted); margin-bottom: 2rem; line-height: 1.5;">${repo.description || '...'}</p>
                    <div style="display: flex; gap: 1rem; flex-wrap: wrap; margin-bottom: 1.5rem;">
                        ${repo.topics ? repo.topics.map(t => `<span style="padding: 4px 12px; background: rgba(59, 130, 246, 0.1); color: var(--accent-blue); border-radius: 100px; font-size: 0.8rem;">#${t}</span>`).join('') : ''}
                    </div>

                    <div style="display: flex; gap: 1rem;">
                        <a href="${repo.html_url}" target="_blank" class="btn-primary" style="padding: 0.5rem 1rem; font-size: 0.9rem; background: var(--primary-gradient);"><i class="fa-brands fa-github"></i> ${viewCodeText}</a>
                        ${repo.homepage ? `<a href="${repo.homepage}" target="_blank" style="padding: 0.5rem 1rem; border: 1px solid var(--border-color); border-radius: 12px; color: var(--text-main); text-decoration: none; font-size: 0.9rem;">Demo Live</a>` : ''}
                    </div>
                    <div style="margin-top: 1.5rem; font-size: 0.75rem; color: var(--text-muted);">
                        <i class="fa-regular fa-calendar"></i> ${updatedText}: ${new Date(repo.updated_at).toLocaleDateString()}
                    </div>
                </div>

                ${netScanner ? `<div class="repo-wave-column">${createNetScannerWavePreview()}</div>` : ''}
                ${centroPlus ? `<div class="repo-wave-column">${createCentroPlusApiPreview()}</div>` : ''}
                ${githubProfile ? `<div class="repo-wave-column">${createGitHubProfilePreview()}</div>` : ''}
                ${etsDam ? `<div class="repo-wave-column">${createEtsDamPreview()}</div>` : ''}
                ${zeeBoard ? `<div class="repo-wave-column">${createZeeBoardPreview()}</div>` : ''}

                <div class="repo-stats-panel" style="background: rgba(0,0,0,0.2); padding: 1.5rem; border-radius: 20px;">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                        <div style="text-align: center; padding: 1rem; background: rgba(255,255,255,0.02); border-radius: 12px;">
                            <i class="fa-regular fa-star" style="color: var(--accent-blue);"></i>
                            <div style="font-size: 1.25rem; font-weight: 700;">${repo.stargazers_count}</div>
                            <small style="color: var(--text-muted);">Stars</small>
                        </div>
                        <div style="text-align: center; padding: 1rem; background: rgba(255,255,255,0.02); border-radius: 12px;">
                            <i class="fa-solid fa-code-branch" style="color: var(--accent-blue);"></i>
                            <div style="font-size: 1.25rem; font-weight: 700;">${repo.forks_count}</div>
                            <small style="color: var(--text-muted);">Forks</small>
                        </div>
                    </div>
                    <div id="langs-${repo.id}">
                        <div style="font-size: 0.8rem; margin-bottom: 0.5rem;">${compositionText}:</div>
                    </div>
                </div>
            `;
            ...
            fetch(repo.languages_url).then(r => r.json()).then(langs => {
                ...
                langDiv.innerHTML += `
                    <div style="display: flex; flex-direction: column; gap: 8px;">
                        ${Object.entries(langs).slice(0, 4).map(([l, v]) => `
                            <div>
                                <div style="display: flex; justify-content: space-between; font-size: 0.7rem; margin-bottom: 4px;">
                                    <span>${l}</span>
                                    <span>${total ? ((v/total)*100).toFixed(1) : '0.0'}%</span>
                                </div>
                                <div style="height: 4px; background: rgba(0,0,0,0.3); border-radius: 100px;">
                                    <div style="height: 100%; width: ${total ? (v/total)*100 : 0}%; background: var(--accent-blue); border-radius: inherit;"></div>
                                </div>
                            </div>
                        `).join('')}
                    </div>
